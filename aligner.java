/*
 * Banded alignment object section of Hot RAD
 * 
 * Base code: Nicholas LaRosa
 * Modifications/Refactoring/Banded: Lauren Assour
 * 
 * Last Modified: 15 August 2014 - LAA
 *
 * Banded pseudocode from: http://lectures.molgen.mpg.de/Algorithmische_Bioinformatik_WS0607/reinert1.pdf
 * 
 */
...
public class aligner {
    ...
    public void load(int scoreM, int scoreMM, int scoreG, int maxSize, int over, int init, int area, int band) {	// prepare the matrices
		int i;
		kband = band;
		maxsize = maxSize;
		testarea = area;
	...
	public boolean align(String refStr, String readStr, double myPercentIdent) {		// the function will return false if there is no match 
	    ...
	    int diff = seq1.length()-seq2.length();
		...
		for(int h = -kband; h < kband+diff; h++) {
		    j = i + h;
			...
			// our match value (+1) will be added only when there is a match according to our current scoring scheme  
			percentValues2[j] = percentValues1[j-1] + percent_Sij;   
			// If inside band                                                
			if ((i-1-j) >= -kband && (i-1-j) <= kband) {   
